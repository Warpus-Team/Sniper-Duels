...
public class GunScript : MonoBehaviourPun
{
    ...
    [Header("Ammo System")]
    [SerializeField] private int maxAmmo = 15;       // Capacidade m�xima do carregador
    [SerializeField] private float reloadTime = 1.5f; // Quanto tempo demora a recarga em segundos
    private int _currentAmmo;                         // Muni��o atual no pente
    private bool _isReloading = false;                // Bloqueia a��es enquanto recarrega
    ...
    private void Start()
    {
        ...
        // Come�a a partida com a arma carregada
    ...
    private void Update()
    {
        // 1. Se estiver recarregando, n�o faz mais nada no Update (bloqueia tiro e nova recarga)
        if (_isReloading)
            return;

        // 2. Comando manual de recarga (Teclado R)
        // S� recarrega se o pente n�o estiver cheio
        ...
        // 4. Verifica se tem muni��o. Se n�o tiver, for�a a recarga autom�tica
        if (_currentAmmo <= 0)
        {
            Debug.LogWarning("<b>[Arma]</b> Sem muni��o! Iniciando recarga autom�tica.");
            StartCoroutine(ReloadAnimate());
            return;
        }

        // 5. Controla a cad�ncia de tiro (Fire Rate)
        if (Time.unscaledTime < _lastFireTime + fireRate)
            return;

        _lastFireTime = Time.unscaledTime;
        _currentAmmo--;
        Debug.Log($"<b>[Arma]</b> Tiro disparado! Muni��o restante: <color=cyan>{_currentAmmo}/{maxAmmo}</color>");
        ...
        // Executa o c�lculo f�sico do tiro
    ...
    private IEnumerator ReloadAnimate()
    {
        ...
        // 1. Salva a posi��o e rota��o LOCAIS exatas de onde a arma est� AGORA
        ...
        // === FASE 1: ABAIXANDO A ARMA (0s at� 1s) ===
        while (elapsed < halfDuration)
        {
            elapsed += Time.deltaTime;
            float percent = elapsed / halfDuration;

            // Interpola suavemente a posi��o e a rota��o locais
        ...
        // Pequena pausa te�rica para simular a troca do pente (opcional)
        _currentAmmo = maxAmmo;
        MainGameView.Instance?.UpdateShot(_currentAmmo, maxAmmo);

        // Reseta o cron�metro para a volta
        elapsed = 0f;

        // === FASE 2: VOLTANDO PARA A POSI��O ORIGINAL (1s at� 2s) ===
        while (elapsed < halfDuration)
        {
            elapsed += Time.deltaTime;
            float percent = elapsed / halfDuration;
